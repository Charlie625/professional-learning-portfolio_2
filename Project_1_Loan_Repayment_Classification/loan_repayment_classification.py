...
def main() -> None:
    ...
    for model_name, model in models.items():
        ...
        result = {
            ...
            "Precision Not Paid Back": round(precision_score(y_test, y_pred, pos_label=0, zero_division=0), 4),
            "Recall Not Paid Back": round(recall_score(y_test, y_pred, pos_label=0, zero_division=0), 4),
            "F1 Not Paid Back": round(f1_score(y_test, y_pred, pos_label=0, zero_division=0), 4),
            "Precision Paid Back": round(precision_score(y_test, y_pred, pos_label=1, zero_division=0), 4),
            "Recall Paid Back": round(recall_score(y_test, y_pred, pos_label=1, zero_division=0), 4),
            "F1 Paid Back": round(f1_score(y_test, y_pred, pos_label=1, zero_division=0), 4),
        }
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "model_comparison_macro_f1.png", dpi=300)
    plt.close()

    plt.figure(figsize=(6, 4))
    class_counts = y.value_counts().sort_index()
    plt.bar(["Not Paid Back", "Paid Back"], [class_counts.get(0, 0), class_counts.get(1, 0)])
